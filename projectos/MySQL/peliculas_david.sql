...
USE DB_MOVIES;
SELECT * FROM directors; -- 1
SELECT name as "nombre_director" from db_movies.directors; -- 2
SELECT * FROM directors ORDER BY name; -- 3
SELECT * FROM movies WHERE title like '%:%'; -- 4
SELECT * FROM stars WHERE name like 'm%'; -- 5
SELECT * FROM stars WHERE name like '%e_'; -- 6
SELECT year FROM movies GROUP BY year; -- 7
SELECT COUNT(DISTINCT year) as TOTAL FROM movies; -- 8
SELECT * FROM movies WHERE title = 'Pulp Fiction'; -- 9
SELECT * FROM movies WHERE id = 6; -- 10
SELECT * FROM movies WHERE id > 3; -- 11
SELECT * FROM movies WHERE id <= 5; -- 12
SELECT * FROM movies WHERE id >= 3 AND id <= 6; -- 13 a
SELECT * FROM movies WHERE id between 3 AND 6; -- 13 b

USE DB_MOVIES;
SELECT * FROM movies WHERE year <> 2013; -- 14 a
SELECT * FROM movies WHERE year != 2013; -- 14 b
SELECT * FROM movies WHERE year = 1895 or year = 1994 or year = 1997; -- 15
SELECT * FROM movies WHERE year != 2013 and year != 2014 and year != 1994; -- 16
SELECT * FROM movies WHERE year >= 2000 or imdb_rating >= 8.5; -- 17
SELECT * FROM movies WHERE year < 2005 or imdb_rating > 8.5; -- 18
SELECT * FROM movies ORDER BY year asc, imdb_rating desc; -- 19 a
SELECT * FROM movies ORDER BY 3 asc, 7 desc; -- 19 b
SELECT * FROM stars limit 3; -- 20
SELECT * FROM movies order by imdb_rating desc limit 5; -- 21
SELECT MIN(imdb_rating) as MIN_RATING, MAX(imdb_rating) AS MAX_RATING FROM movies; -- 22
SELECT AVG(imdb_rating) as RATING_AVG FROM movies; -- 23
SELECT ROUND(AVG(imdb_rating), 2) as RATING_AVG FROM movies; -- 24

USE DB_MOVIES;
SELECT COUNT(*) as total_directores FROM directors; -- 25
SELECT ROUND(SUM(imdb_rating),2) as RATING_POINTS FROM movies; -- 26
SELECT * FROM movies WHERE imdb_rating > (SELECT AVG(imdb_rating) FROM movies); -- 27
SELECT * FROM movies WHERE id NOT BETWEEN 2 AND 4; -- 28

SELECT  movies.title,directors.name from movies inner join movies_directors on movies.id = movies_directors.movies_id 
INNER JOIN directors on movies_directors.directors_id = directors.id; -- 29

SELECT mov.title, dir.name FROM movies mov 
INNER JOIN movies_directors mov_dir
ON mov.id = mov_dir.movies_id
INNER JOIN directors dir
ON mov_dir.directors_id = dir.id; -- 29 con alias

SELECT mov.title, star.name from movies mov inner join movies_stars mov_star
on mov.id = mov_star.movies_id inner join stars star on mov_star.stars_id = star.id; -- 30

SELECT mov.title, gen.name from movies mov inner join movies_genres mov_gen
on mov.id = mov_gen.movies_id inner join genres gen on mov_gen.genres_id = gen.id; -- 31

SELECT mov.title, gen.name as genre, star.name as actor, dir.name as director from movies mov inner join movies_genres mov_gen
on mov.id = mov_gen.movies_id inner join genres gen on mov_gen.genres_id = gen.id
inner join movies_stars mov_star on mov.id = mov_star.movies_id inner join stars star on mov_star.stars_id = star.id 
INNER JOIN movies_directors mov_dir ON mov.id = mov_dir.movies_id INNER JOIN directors dir ON mov_dir.directors_id = dir.id order by mov.title; -- 32

SELECT mov.title, count(gen.name) as genres from movies mov inner join movies_genres mov_gen
on mov.id = mov_gen.movies_id inner join genres gen on mov_gen.genres_id = gen.id group by mov.title; -- 33

SELECT dir.name, count(mov.title) as total_peliculas FROM movies mov INNER JOIN movies_directors mov_dir ON mov.id = mov_dir.movies_id
INNER JOIN directors dir ON mov_dir.directors_id = dir.id group by dir.name; -- 34

SELECT 
    mov.title, 
    CASE 
        WHEN mov.imdb_rating BETWEEN 5 AND 6 THEN 'Suficiente' 
        WHEN mov.imdb_rating BETWEEN 6 AND 7 THEN 'Bien'
        WHEN mov.imdb_rating BETWEEN 7 AND 8 THEN 'Notable'
        WHEN mov.imdb_rating BETWEEN 8 AND 10 THEN 'Excelente'
        ELSE CAST(mov.imdb_rating as CHAR)
    END as imdb_rating
FROM movies mov; -- 35
